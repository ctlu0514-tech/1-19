...
# --- ❌ 删除结束 ---
# 修改为您宁波数据的实际路径
image_path = r'/nfs/lyz/rc_data/ningbodata/zqnii'  # 图像文件夹
mask_path = r'/nfs/lyz/rc_data/ningbodata/zqroi'   # 掩膜文件夹
print("正在搜寻图像和掩码...")
patient_list = []
patient_mask_list = []
patient_id_list = []
...
for image_filename in image_files:
    ...
    # --- ✅ 粘贴新代码 ---
    # 假设 image_filename 是 '1332349.nii'
    base_name = image_filename.split('.nii')[0]  # 得到 '1332349'

    # 宁波数据的掩膜规律：文件名 + .nii.gz
    # 拼凑出 '1332349.nii.gz'
    mask_filename_gz = f"{base_name}.nii.gz"
    mask_full_path = os.path.join(mask_path, mask_filename_gz)

    # 兼容性写法：如果 .nii.gz 找不到，再试试 .nii
    if not os.path.exists(mask_full_path):
        mask_filename_nii = f"{base_name}.nii"
        mask_full_path = os.path.join(mask_path, mask_filename_nii)
    # -------------------
...
df.to_csv(r'/data/qh_20T_share_file/lct/CT67/ningbo_ovarian_features.csv', index=0)
print('Done')
