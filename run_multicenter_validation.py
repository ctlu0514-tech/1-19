...
# 1. 数据加载与划分模块 (保持不变)
# ===================================================================
def load_and_split_data(csv_path, train_center_names, label_col='type', center_col='Center', id_col='ID'):
    ...
    # --- 方法 4: CDGAFS ---
        ...
        try:
            (sel_idx, _, _, _, _) = cdgafs_feature_selection(
                ...
                theta=0.9, omega=0.5, population_size=100,
...
def main():
    # --- 配置 ---
    CSV_FILE = '/data/qh_20T_share_file/lct/CT67/ovarian_covbat_FuYi.csv'
    TRAIN_CENTERS = ['FuYi']  # 可修改为多个中心 ['FuYi', 'FuEr', 'ShiZhongXin']
    ...
    for method in METHODS:
        ...
        for ds_name, res in scores.items():
            ...
            row[f"{ds_name}_Sens"] = res['Sens']  # 新增
            row[f"{ds_name}_Spec"] = res['Spec']  # 新增
